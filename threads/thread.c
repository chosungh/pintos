...
void thread_sleep(int64_t ticks)
{
	...
	cur->wkup_tick = ticks;
	list_insert_ordered(&sleep_list, &cur->elem, thread_cmp_criterion, NULL);
	thread_block();

	intr_set_level(old_level);
}

void thread_wkup(int64_t ticks)
{
	enum intr_level old_level;
...
bool thread_cmp_criterion(const struct list_elem *a, const struct list_elem *b, void *aux UNUSED)
{
	...
	return sa->wkup_tick < sb->wkup_tick;
}
